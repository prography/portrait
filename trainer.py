...
"""
최윤제씨 코드에는 없는데, 실제 학습 시에는
loss plot 띄우기 위해서 visdom 사용할 수도 있어서 일단 import
"""
from vis_tool import Visualizer

class Trainer(object):
    """
    우리는 dataloader가 데이터셋 별로 필요한 게 아니고, train/test 별로 필요한 거라서 인자 수정.
    """
    def __init__(self, train_loader, test_loader, config):
        ...
        """2번째 dataset의 이미지들 dimension인데 필요 없으니까 삭제"""
        ...
        """CelebA에서만 쓰일 selected attributes라서 삭제"""
    ...
    def build_model(self):
        """
        우리는 RaFD 타입의 dataset만 쓸거라서, CelebA 관련 코드는 신경 안써도 됨.
        """
        ...
        """
        self.dataset이 'Both'인 경우는 2개의 dataset 가지고 학습하는 경우.
        우리 프로젝트에선 쓰이지 않을 부분이라 삭제
    ...
    """
    우리는 attribute를 선택해서 사용하지 않을거라 selected_attrs 인자 필요 없음.
    """
    def create_labels(self, c_org, c_dim=5, dataset='Portrait'):
        """
        마찬가지로 아래 attribute name 설정해주는 부분도 삭제.
    ...
    def classification_loss(self, logit, target, dataset='Portrait'):
        """
        CelebA 관련 loss function 정의 부분 삭제
    ...
    def train(self):
        ...
        """
        data loader 설정 역시 하나만 들어오니까 dataset 따라 다르게 설정하는 부분 삭제.
        ...
        """create_labels에서 attribute select 하는 인자가 빠졌으니 호출 시에도 삭제"""
    ...
    """
    아래의 train_multi 함수는 2개의 dataset을 가지고 학습 시에 필요한 함수.
    일단 우리 프로젝트에선 쓰이지 않으므로 삭제.
    ...
    """
    test_multi 역시 2개의 dataset 가지고 학습 시 필요
