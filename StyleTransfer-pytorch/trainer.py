        ...
        for epoch in range(self.nepochs):
            for step, (content, _) in enumerate(self.dataloader):
                ...
                if (step+1) % self.sample_interval == 0:
                    img = output_img.cpu()
                    img = img[0]
                    outpath = os.path.join(self.sample_folder, 'output_epoch%d.png' % (epoch+1))
                    utils.save_image(outpath, img)
                    print("[*] Save sample image completed!")

            # do checkpointing
            if (epoch+1) % self.checkpoint_interval == 0:
                self.tfNet.eval()
                outpath = os.path.join(self.sample_folder, 'ckpt_epoch%d.pth' %(epoch+1))
                torch.save(self.tfNet.state_dict(), outpath)
                print("[*] Save checkpoint completed!")

        print("Learning finished!!!")
        outpath = os.path.join(self.sample_folder, 'final_epoch%d.pth' % self.nepochs)
        torch.save(self.tfNet.state_dict(), outpath)
        print("[*] Save final model completed!")